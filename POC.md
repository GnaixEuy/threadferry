...
- [ ] 打开“日志追踪”，按错误编号、Agent、动作或资源定位记录，确认页面只显示脱敏失败状态和 Activity，
      不显示消息正文；点击“上报问题”后系统浏览器打开 GitHub Issue，标题、运行环境和当前筛选日志已预填；
      点击“清理日志”并确认后两类记录消失，任务状态、Session、待补发、提醒和配置保留。
...
- [ ] 确认 Runtime 无法直接调用 `wecom-cli`，也无法提交 `auth`、`identity`、任意 shell、凭据字段、
      输出路径或本地文件路径。
- [ ] 模拟通讯录返回未授权、完整会话数据返回企业未开通、最近会话正常，确认机器人自动显示“兼容模式”，
      长连接和普通 Agent 私聊仍可运行，`doctor` 不把可选权限缺失判为整体故障。
- [ ] 在新增或重新授权机器人后，确认管理台弹窗显示“完整模式”“兼容模式”或“检测中”；模拟网络失败时
      只显示“检测中”，不得误判为兼容模式。
- [ ] 先让权限探测返回未授权，再改为全部可用，确认后台定期探测会自动从兼容模式切回完整模式，无需
      重启或重新授权。
- [ ] 通讯录不可用且 Owner 身份需要跨命名空间映射时，确认回复引导使用一次性私聊配对，不把普通文本
      误报为资源下载失败，也不自动信任第一个私聊者。
- [ ] 完整群历史不可用时发送不依赖前文的群 `@` 请求，确认 Runtime 使用当前消息和本机历史继续处理；
      请求总结此前普通群聊时，确认回复明确说明无法读取且不编造上下文。
- [ ] 模拟日程、会议或文档真实查询返回权限不足，确认只拒绝该项企业操作并给出对应能力提示，后续普通
      Agent 对话继续可用。
