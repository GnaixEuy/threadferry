...
| 4 | N 条连接并发运行 + 入站路由 + 进程内作用域隔离 | ✅ 完成（双机器人已真机验证） |
| 5 | Owner 与授权按 agent；私聊直达该 agent | ✅ 完成（由单 Agent 视图达成） |
| 6 | 管理台、文档、发版 | ✅ 完成（0.16.0 / 0.17.0 已发布） |
...
- 不做跨 agent 的 Session、凭据或群历史共享。显式协作任务只复制任务说明和执行结果，并把它们标为不可信数据。
- 不做隐式的 agent 自由编排；只有 Owner 明确创建的持久协作任务可以交接和复核。
...
- [x] WebSocket 从只监听 `message.text` 改为统一消息入口，接收文本、图片、图文、语音、文件、视频，
  并下载当前消息与引用消息中的图片和文件。
- [x] 群历史不再丢弃 `media_id`；仅在分析轮通过 `message files get` 补取最近资源，新鲜度复查只读
  元数据，避免重复下载。
- [x] Runtime 统一接收短生命周期资源：Codex/Pi/Grok 使用原生图片输入，Claude 使用受控 `--add-dir`，
...
- [x] `processDirect` 不再传空历史；私聊与群聊统一调用 `chat messages list`，并按会话类型合并
  所属 Agent 的本机历史。
- [x] 兼容 `message files get` 实测顶层 `file_path`、Schema 中的 `media_item.file_path` 与
  `media_item.content`，并解析顶层 `error.code` 错误结构。
- [x] 每个 Agent 持久保存自己实际收到的授权消息和资源，私聊/群聊分区，保留 7 天、1,000 条、
...
- 私聊与群聊都会调用远端历史；私聊远端失败或返回空时，使用所属 Agent 的 7 天本机历史，当前消息
  仍可正常处理，不再依赖群历史权限才能使用私聊。
- 群聊远端历史失败时自动进入兼容模式，继续使用当前 `@机器人` 消息和本机已授权历史；Host 向 Runtime
  注入可信能力说明，请求依赖此前普通群消息时必须明确说明无法读取，不能把本机历史冒充完整上下文。
- 本机历史只记录通过 Owner/群授权并实际触发 Agent 的消息，不被动收集普通群消息，也不跨 Agent
  共享。
- 通讯录、完整会话历史和最近会话在配对时按 Agent 探测并弹窗提示，运行期间每 5 分钟重试；只有明确
  缺失时进入兼容模式，探测异常保持检测状态，权限恢复后自动切回完整模式。日程、会议、文档等业务
  权限在首次真实调用时判断，缺失时只停用对应能力，不影响 Runtime 的本地文件、命令、网络和自动化权限。
