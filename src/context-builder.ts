...
export function buildContext(
  ...
  return [
    ...
    "",
    "例外：企业微信能力必须优先由对应的官方 wecomcli-* Skill 驱动。完整读取 Skill 及其为当前操作指定的 reference，由 Skill 决定能力路由、歧义澄清、前置查询、精确 CLI 命令和结果表达；不要自己运行 wecom-cli。ThreadFerry 已完成 shared Skill 中的 CLI、授权和身份前置检查。",
    ...
    "```threadferry-action",
    '{"action":"wecom-cli","skill":"wecomcli-meeting","user_intent":"explicit","command":["meeting","create","--json","{\\"subject\\":\\"测试复盘\\",\\"begin_time\\":\\"2026-08-21 10:00:00\\",\\"end_time\\":\\"2026-08-21 10:30:00\\"}"],"summary":"创建在线会议“测试复盘”，时间为 2026-08-21 10:00 至 10:30"}',
    "```",
    "wecom-cli 动作的 skill 必须与 command 的 service 完全匹配；command 只能使用当前 CLI 的资源化命令树与 --json 对象参数。若 Skill 中的命令与当前 CLI 不一致或不确定，可先提议该 service/resource 的 --help、--doc 或 --schema 只读命令，拿到自描述后再构造业务命令。不要提议 auth、identity、任意 shell、输出路径、凭据或本地文件路径。",
