...
ThreadFerry 会在机器人配对时探测通讯录、完整会话历史和最近会话能力，并弹窗显示结果。企业明确没有
批准其中某项权限时，
机器人自动进入兼容模式：当前消息、本机 Workspace 和 Runtime 自动化继续可用；按姓名找人、读取完整
群上下文等受限功能会单独停用并给出准确提示。日程、会议、邮件、文档等业务能力在实际使用时按企业
返回的权限自动降级；无法确认权限时保持检测状态，后台定期重试，权限恢复后自动切回完整模式。
...
并可把当前筛选结果预填到 GitHub Issue；入口可以在偏好设置中隐藏。偏好设置还可以切换主题，并控制登录时启动、服务自动启动、启动后打开
