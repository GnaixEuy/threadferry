...
管理台分为「概览」「Agent 工作区」「群聊管理」三个页面：概览页汇总运行状态和待处理事项；Agent 工作区页可以新增（Workspace 支持浏览本机目录选择）、查看绑定关系并删除未被使用的工作区；群聊管理页可以绑定或解绑群、切换 Agent 工作区、切换全员可用开关、管理可使用用户和重置群 Session。修改立即生效。
