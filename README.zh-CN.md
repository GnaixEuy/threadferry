...
Agent 本身是**只读**的：沙箱不给网络和 shell，也不继承环境变量。但它能**识别写操作意图并给出
建议**，由你确认后 ThreadFerry 代为执行：

```text
你：@机器人 帮我建个日程，明天上午十点，关于刚才的测试
机器人：我建议这样安排。
...
你（私聊）：threadferry confirm 918F46
机器人：已执行：动作：创建日程 …
```

要点：

- **只有白名单里的动作能被执行**，目前是 `schedule.create`（创建日程）。动作名之外的提议一律
  当作没提议。
- **必须 Owner 在私聊里确认**。确认码一次性、10 分钟过期、只存在内存里。非 Owner 连确认命令都
  走不到。
- 群历史仍然是不可信输入：提示词注入最多让 Agent 提议一个动作，参数还要过校验、还要你亲自确认。
- 参与人按姓名或 `id:<userid>` 经通讯录解析。信息不全时 Agent 会先问清楚，不会臆造时间。
- 想加动作（待办、文档、邮件等）在 `src/actions.ts` 的白名单表里加一项即可，确认与执行流程自动复用。
