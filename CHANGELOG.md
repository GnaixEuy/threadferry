...
群聊详情改为名称优先，并重新整理机器人状态与管理操作的布局。

### 主要变化

- 群名称、群列表和授权成员默认不再铺开内部 ID，悬停或键盘聚焦名称时才显示排障信息。
- 群详情去除重复标题；机器人启停与全员开关并列，成员操作保持就近，Session 与移除操作分组对齐。
- 窄屏下状态操作、成员操作和危险操作按层级换行，保留完整点击区域。

### 安装与升级

直接升级并重新启动 ThreadFerry 即可；配置、凭据、授权关系和 Session 均不受影响。

## 0.32.0

Runtime 不再被 ThreadFerry 强制降级为只读分析器，恢复各本地 Agent 原生的项目指令与自动化能力。

### 主要变化

- Codex 重新加载用户配置、`AGENTS.md`、规则、插件和原生权限，因此不再触发自定义沙箱拦截 CLI 自身的问题。
- Pi 重新加载 `AGENTS.md` / `CLAUDE.md`、Skills、扩展和内置工具；Claude Code 与 Grok Build 同样遵循各自的原生配置。
- ThreadFerry 仍负责机器人身份、会话和凭据隔离，以及企业微信动作的 Broker 校验与确认；本地文件、命令和网络权限改由各 Runtime 自己管理。

### 安装与升级

直接升级并重新启动 ThreadFerry 即可；请只为可信用户和群开放机器人，并在各 Runtime 中配置符合需要的权限策略。

## 0.31.1

管理台默认隐藏不便识别的企业微信内部 ID，机器人卡片和授权成员名单改为名称优先。

### 主要变化

- Bot ID、Owner userid 和群 ID 不再直接铺在机器人卡片上，悬停或键盘聚焦对应名称时按需显示。
- 已知姓名的群成员同样按需显示 userid；无法取得姓名时仍直接显示 ID，避免成员无法辨认。
- 浮层沿用现有亮色、暗色、高对比度和减少动态效果设置，无需新增脚本或依赖。
...
机器人现在可以直接在管理台修改 Workspace，无需删除后重新添加。

### 主要变化

- 机器人卡片新增“修改工作区”入口，复用现有目录选择器，也支持直接输入本机绝对路径。
- 保存前继续校验路径必须存在、必须是目录且不能通过符号链接逃逸；校验失败时保留已输入路径并重新打开对话框。
- Workspace 变化后，后续任务按新的 `Agent + Runtime + Workspace` 作用域建立独立 Session，不会误续接旧工作区会话。
- 无脚本目录浏览页同样支持为指定机器人选择新工作区。

### 安装与升级

直接升级并重新启动 ThreadFerry 即可；机器人凭据、群聊、授权名单、Runtime Session 和本机历史均不受影响。

[查看 v0.30.3...v0.31.0 的完整变更](https://github.com/GnaixEuy/threadferry/compare/v0.30.3...v0.31.0)

## 0.30.3

修正群详情中状态操作按钮宽度不一造成的视觉错位。

### 主要变化

- 机器人“停用/启用”和全员可用开关使用一致的右侧操作列与按钮宽度，不再因文字长短产生左边缘跳动。
- 操作列继续保持右对齐；页面宽度不足发生换行时，按钮仍落在同一视觉基线上。

### 安装与升级

直接升级并重新启动 ThreadFerry 即可；机器人、群聊、授权名单、Workspace、Runtime Session 和本机历史均不受影响。

[查看 v0.30.2...v0.30.3 的完整变更](https://github.com/GnaixEuy/threadferry/compare/v0.30.2...v0.30.3)

## 0.30.2

机器人管理页始终显示删除入口；存在群聊绑定时，改为明确引导到群详情解除绑定，避免把“停用”误当成“解绑”。

### 主要变化

- 每张机器人卡片始终显示“删除机器人”，不能删除时在对话框内说明原因和下一步。
- 仍绑定群聊时列出对应群，并提供“去解除绑定”入口；停用的群聊绑定也必须先正式移除。
- 仅剩一台机器人时保留删除入口，并提示先添加另一台机器人。

### 安装与升级

直接升级并重新启动 ThreadFerry 即可；配置、机器人凭据、Workspace、Runtime Session 和本机历史均不受影响。

## 0.30.1

管理台改用更克制的桌面工具视觉，减少卡片、渐变和装饰性色彩带来的模板感，同时让状态信息更容易扫读。

### 主要变化

- 侧栏、页面标题、统计区、内容卡片和表单控件统一为中性色层级与细边框，去除品牌渐变、卡片阴影和
  重复标题，保留少量绿色只表达主操作与运行状态。
- 十个独立统计卡收拢为一个连续信息面，导航移除含义不一致的装饰符号，缩短页面视觉路径。
- 管理台中的提醒、协作任务、执行结果和 Runtime Session 改用中文状态文案。
- 补充减少动态效果、减少透明度和增强对比度的系统偏好适配，亮色、暗色与窄屏布局继续可用。

### 安装与升级

直接升级并重新启动 ThreadFerry 即可；配置、机器人凭据、Workspace、Runtime Session 和本机历史均不受影响。

[查看 v0.30.0...v0.30.1 的完整变更](https://github.com/GnaixEuy/threadferry/compare/v0.30.0...v0.30.1)

## 0.30.0

让桌面应用自行完成检查、下载、安装和重启，后续升级不再需要用户前往 GitHub 手动下载安装包。

### 主要变化

- 桌面应用启动后自动检查更新，并每 6 小时复查一次；偏好设置保留“立即检查更新”入口和实时下载进度。
- 使用桌面安装器的原生更新链路，在应用私有缓存中下载构建时生成 SHA-512 元数据的更新包，下载完成后
  自动安装并重新启动，不再把安装包放进系统“下载”目录或要求用户手动打开。
- 安装前停止接收新任务并等待当前 Runtime、补发和持久化工作安全结束；若安装器启动失败，会恢复服务并
  在偏好设置与日志中显示原因。
- Release 新增 Electron 更新元数据、blockmap 和 macOS ZIP；macOS Release 构建强制使用 Developer ID
  签名和 Apple 公证，避免发布无法被自动更新器接受的安装包。
- 从普通浏览器访问管理台时仍只检查版本；自动下载、安装与重启只由受限的桌面主进程执行。

### 安装与升级

现有版本需要最后一次手动安装 ThreadFerry 0.30.0。以后保持桌面应用运行即可自动升级；系统需要权限时
仍可能显示系统级确认。现有配置、机器人凭据、Workspace、Runtime Session 和本机历史不受影响。

[查看 v0.29.1...v0.30.0 的完整变更](https://github.com/GnaixEuy/threadferry/compare/v0.29.1...v0.30.0)

## 0.29.1

补齐群机器人的停用、重新接入和移除能力，并修复移除后的残留授权、自动任务和口语误判，
确保“已移除”真正停止该 Agent 的群行为。

### 主要变化

- Owner 私聊新增 `threadferry disable`、`enable` 和 `unbind`；“机器人我想移除”等明确口语请求会被识别，
  并要求用显式命令确认，避免误删。
- 管理台群详情新增“移除机器人”，会清理该 Agent 在该群的授权和 Runtime Session；持久化的已移除
  状态可阻止群发现或再次 `@` 自动接回，并支持一键重新接入。
- 界面和文档明确区分 ThreadFerry 绑定与企业微信群成员关系；从群成员中移除机器人仍需群管理员在
  企业微信中操作。
- 已移除群拒绝旧邀请码、成员授权和全员开关变更；重新接入后必须重新生成邀请码。
- 移除时取消该 Agent 在群里的提醒并丢弃待发送消息；调度、协作任务、主动通知和崩溃恢复也会在
  投递前再次检查移除状态。
- 收紧“移除机器人”口语识别，只匹配明确操作意图，不再把“分析机器人移除功能的代码”等普通任务
  错当成管理命令。

### 安装与升级

升级并重新启动 ThreadFerry 即可；现有配置自动兼容，已移除群的状态会继续生效。

[查看 v0.28.0...v0.29.1 的完整变更](https://github.com/GnaixEuy/threadferry/compare/v0.28.0...v0.29.1)
